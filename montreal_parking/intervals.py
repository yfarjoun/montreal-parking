...
def _classify_level_interval(
    ...
    if not active_signs:
        return None
    if level == 3:
        if any(s["category"] in (SignCategory.NO_PARKING, SignCategory.PERMIT) for s in active_signs):
            return IntervalCategory.RESTRICTED
        return None
    if level == 4:
        ...
        if SignCategory.UNRESTRICTED in categories:
            return IntervalCategory.FREE
    return None
...
def _merge_level_spans(
    ...
    for i in range(len(sorted_bounds) - 1):
        ...
        if l4_cat is not None:
            ...
            final_cat = IntervalCategory.FREE
...
def _build_side_intervals(
    ...
    for start, end, cat, descs in merged:
        ...
        if cat == IntervalCategory.FREE and start == 0.0 and end < MIN_FREE_EDGE_M:
            cat = IntervalCategory.NO_DATA
        if cat == IntervalCategory.FREE and end == road_length and (road_length - start) < MIN_FREE_EDGE_M:
            cat = IntervalCategory.NO_DATA
...
def reconstruct_intervals(
    ...
    for (id_trc, side_key), group in grouped:
        ...
        for _, sign_row in group.iterrows():
            if sign_row["sign_category"] == SignCategory.STREET_CLEANING:
                continue
    ...
    for id_trc in covered_segments:
        ...
        for fill_side in ("left", "right"):
            ...
            intervals.append({
                ...
                "category": IntervalCategory.NO_DATA,
                "street_name": street_name,
